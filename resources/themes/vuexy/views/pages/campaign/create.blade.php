		...
		document.addEventListener('DOMContentLoaded', function () {
			...
			document.querySelector('.btn-submit').addEventListener('click', function () {
				fv3.validate().then(function (result) {
					...
					const staticFields = document.querySelectorAll('#step-1 input, #step-1 select, #step-1 textarea, #step-2 > .form-group > select, #step-3 input, #step-3 select, #step-3 textarea');
					...
					if (visibleForm) {
						visibleForm.querySelectorAll('input, select, textarea').forEach(el => {
							if (el.name) formData.append(el.name, el.value);
						});
					}
