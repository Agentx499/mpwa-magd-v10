...
class ThemesController extends Controller
{
    public function index()
    {
            ...
            foreach ($folders as $folder) {
                ...
                if (File::exists($infoFile)) {
                    ...
                    $themes[] = [
						...
						'compatibility' => $info['compatibility'] ?? 'Unknown',
		...
		$checkUrl = "https://mpwa.onexgen.com/tools/themes.php?v=$currentVersion&lang=".config('app.locale')."&rand=".$randomNumber;
	...
	public function downloadTheme(Request $request)
    {
		...
		if (File::exists($newFolder.'/info.json')) {
			...
			File::copyDirectory($newFolder.'/views', resource_path('themes/'.$downloadFolder.'/views/'));
			Storage::deleteDirectory($pathFolder.'/views');
			File::copyDirectory($newFolder, public_path('themes/'.$downloadFolder));
